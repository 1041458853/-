...
<thead>
    <tr>
        ...
        <th width="120" class="<?php echo ns_sorting('author'); ?>" name="author"><?php echo fc_lang('录入作者'); ?></th>
        <th width="160" class="<?php echo ns_sorting('updatetime'); ?>" name="updatetime"><?php echo fc_lang('更新时间'); ?></th>
        <th width="80" style="text-align: center" class="<?php echo ns_sorting('status'); ?>" name="status"><?php echo fc_lang('状态'); ?>&nbsp;&nbsp;</th>
    ...
    <tr class="dr_row_hide" id="dr_row_<?php echo $t['id']; ?>">
        ...
        <td style="text-align: center"><label><?php if ($t['status'] == 9) { ?><a class="btn blue btn-xs" href="javascript:dr_status('<?php echo $t['id']; ?>', '<?php echo $t['status']; ?>');"><?php echo fc_lang('正常'); ?></a><?php } else { ?><a class="btn red btn-xs" href="javascript:dr_status(<?php echo $t['id']; ?>, '<?php echo $t['status']; ?>');"><?php echo fc_lang('关闭'); ?></a><?php } ?></label></td>
        <td>
    ...
    <tr class="mtable_bottom">
        ...
        <td colspan="99" >
            <?php if (!$get['flag']) {  if ($this->ci->is_auth(APP_DIR.'/admin/home/del')) { ?><label><button type="button" class="btn red btn-sm" name="option" onClick="$('#action').val('del');dr_confirm_set_all('<?php echo fc_lang('删除之后将无法恢复，您确定要这样操作吗？'); ?>')"> <i class="fa fa-close"></i> <?php echo fc_lang('删除'); ?></button></label><?php }  if ($this->ci->is_auth(APP_DIR.'/admin/home/del')) { ?><label><button type="button" class="btn dark btn-sm" name="option" onClick="$('#action').val('recycle');dr_confirm_set_all('<?php echo fc_lang('将内容放入回收站，您确定要这样操作吗？'); ?>')"> <i class="fa fa-trash"></i> <?php echo fc_lang('回收站'); ?></button></label><?php }  } else {  if ($this->ci->is_auth(APP_DIR.'/admin/home/edit')) { ?><label><button type="button" class="btn red btn-sm" name="option" onClick="$('#action').val('flag');dr_confirm_set_all('<?php echo fc_lang('您确定要这样操作吗？'); ?>')"> <i class="fa fa-flag"></i> <?php echo fc_lang('移出'); ?></button></label><?php }  }  if ($this->ci->is_auth(APP_DIR.'/admin/home/edit')) { ?>

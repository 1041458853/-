...
class Excel extends M_Controller {
    ...
    public function __construct() {
        ...
        if (!$this->tableinfo) {
            ...
            $this->tableinfo = $this->system_model->cache(); // 表结构缓存
        }

        $this->template->assign(array(
            'menu2' => $this->get_menu_v3(array(
                fc_lang('Excel导出') => array(APP_DIR.'/admin/excel/index', 'reply-all'),
                fc_lang('Excel导入') => array(APP_DIR.'/admin/excel/import', 'sign-in'),
            )),
    ...
    public function index() {
        ...
        if ($action == 'export') {
            ...
            if (IS_POST) {
                ...
                !$ids && $this->admin_msg(fc_lang('无可用字段'));
                foreach ($ids as $id) {
                    $field[] = $id;
                    $name[] = $data[$id]['name'];
                    $func[] = $data[$id]['func'];
                ...
                !$count && $this->admin_msg(fc_lang('查询导出结果为空'));
                !$total && $total = $count;
            ...
            if (IS_POST) {
                ...
                if (!$this->tableinfo[$table]) {
                    $this->admin_msg(fc_lang('数据表不存在'));
                } elseif (!$this->tableinfo[$table]['rows']) {
                    $this->admin_msg(fc_lang('数据表中没有任何记录'));
                }
                $this->admin_msg(fc_lang('准备中...'), $this->url('excel/index', array('action' => 'export', 'table'=>$table)), 2);
            }

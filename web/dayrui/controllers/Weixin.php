...
class Weixin extends M_Controller {
	...
	public function sync() {
		...
		$back = urldecode($this->input->get('url'));
...
/*                $url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$this->wx['config']['key'].'&redirect_uri='.urlencode(SITE_URL.'index.php?c=weixin&m=member').'&response_type=code&scope=snsapi_base&state='.urlencode($url).'#wechat_redirect';
        		...
        		$code = $this->input->get('code');
        		if (!$code) {
        		    redirect('https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$this->wx['config']['key']
        		        .'&redirect_uri='.urlencode(dr_now_url()).'&response_type=code&scope=snsapi_base#wechat_redirect');
        		    die;
        		}
        		$data = json_decode(dr_catcher_data('https://api.weixin.qq.com/sns/oauth2/access_token?appid='.$this->wx['config']['key'].'&secret='.$this->wx['config']['secret'].'&code='.$code.'&grant_type=authorization_code'), true);
        		//die(json_encode($data));
        		if (isset($data['errcode']) && $data['errcode']) {
        		    $this->msg('微信授权出错：' . $data['errcode']);
        		}
        		if ($this->weixin_model->_init_user($data['openid'])) {
        		    redirect($back, 'refresh');
        		} elseif (empty($_GET['login_fail'])) {
        		    redirect($back . (strpos($back, '?') ? '&' : '?') . 'login_fail=1', 'refresh');
        		} else {
        		    $this->msg('登录授权失败！');
        		}
            }
	...
	private function _init_user() {
	    if ($this->data['EventKey']) {
    	    parse_str(substr($this->data['EventKey'], strlen('qrscene_')), $event_key);
	    }
        $this->weixin_model->_init_user($this->data['FromUserName'], empty($event_key) ? null : $event_key);
	}

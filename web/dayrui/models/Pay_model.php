...
class Pay_model extends CI_Model{
    ...
    public function pay_success($sn, $money, $note = '') {
        ...
        //如果此用户有邀请人,支付成功后根据邀请人的id在menber使其优惠券个数加一
        $inviter = $this->db->get_where('member', array('uid' => $uid))->row_array()['inviter'];
        if($inviter != null){
            $this->db->set('youhuiquan', 'youhuiquan+1', FALSE);
            $this->db->where('uid', $inviter);
            $this->db->update('member');
        }
        
        //如果此用户有使用优惠券,支付成功后使其优惠券个数减一
        $youhuiquan = $this->db->get_where('member_paylog', array('id' => $id))->row_array()['youhuiquan'];
        if($youhuiquan != null){
            $this->db->set('youhuiquan', 'youhuiquan-1', FALSE);
        ...
        if ($data['module']) {
            ...
            }elseif ($data['module'] == 'mall') {
                $this->db->where('dingdanhao',$id)->update("zengsongjilu",array('status' => 1));
            }
        }
    ...
    public function add_for_online($pay, $money, $module = APP_DIR, $order = array()) {
        ...
        $this->db->insert('member_paylog', array(
            ...
            'type' => $pay,
            'value' => $money,
            'order' => $order[0],//dr_array2string($order),
            'status' => 0,
            'module' => $module,
            'inputtime' => SYS_TIME,
            'mall_id' => $order[0], //课程id
            'chiyouren_name' => $order[1],
            'chiyouren_phone' => $order[2],
            'goumaishuliang' => $order[3],
            'shengyushuliang' => $order[5],
            'beizhu' => $order[4],
            'youhuiquan' => $order[6]
        ));
        ...
        if($order[5]){
            //更新赠送记录表
            $this->db->insert('zengsongjilu', array(
                'zengsongren_id	' => $this->uid,
                'jieshouren_id' => $order[5],
                'kecheng_id' => $order[0],
                'dingdanhao' => $id,
                'time' => SYS_TIME,
                'status' => 0

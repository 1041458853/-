...
class Home extends M_Controller {
    ...
    public function price() {
        ...
        if (!$data) {
            echo dr_json(0, fc_lang('参数数据错误'));exit;
        }
        ...
        if ($is_shipping) {
            ...
            if (!$city) {
                echo dr_json(0, fc_lang('未选择收货城市'));exit;
            } elseif (!dr_linkage('address', $city)) {
                echo dr_json(0, fc_lang('收货城市不存在'));exit;
            }
        } else {
            $city = -1;
        }
    ...
    public function index() {
        ...
        if (!@in_array($this->groupid, $this->mconfig['permission']['buy']['use'])) {
            if ($this->groupid) {
                $this->msg(fc_lang('当前用户无权限下单'));
            } else {
                $this->msg(fc_lang('没有登录，无权限下单'), dr_member_url('login/index'), 3, 2);
            }
        }
        ...
        if ($oids) {
            ...
            if (!$data) {
                $this->msg(fc_lang('订单中无可用商品'));
            }
            foreach ($data as $t) {
                if (!isset($this->mconfig['module'][$t['mid']])
                    || !$this->mconfig['module'][$t['mid']]['use']) {
                    $this->msg(fc_lang('模块【%s】没有开启电商功能', $t['mid'])); // 未开启电商功能
                }
                // 物流验证
            ...
            if (!isset($this->mconfig['module'][$mid])
                ...
                $this->msg(fc_lang('模块【%s】没有开启电商功能', $mid)); // 未开启电商功能
            }
            // 物流验证
        ...
        if (IS_POST) {
            ...
            if (!isset($pay_type[$post['pay_type']]) || !$pay_type[$post['pay_type']]) {
                $this->msg(fc_lang('付款方式不存在，请选择一个有效的付款方式'));
            }
            if ($is_shipping) {
                $city = $post['shipping_city'];
                if (!$city) {
                    $this->msg(fc_lang('未选择收货城市'));
                } elseif (!dr_linkage('address', $city)) {
                    $this->msg(fc_lang('收货城市不存在'));
                }
            } else {
                $city = -1;
            }
            ...
            foreach ($list as $u => $store) {
                ...
                foreach ($store['goods'] as $t) {
                    ...
                    // 删除对应的购物车商品
                    if (isset($t['id']) && $t['id']) {
                        $this->db->where('uid', $this->uid)->where('id', $t['id'])->delete(SITE_ID.'_order_cart');
                    }
                }
                // 附件归表
                if ($field) {

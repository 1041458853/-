...
class Cart extends M_Controller {
	...
	public function delete() {
        ...
        $table = $this->order_model->prefix.'cart';
        $data = $this->db->where('id', (int)$this->input->get('id'))->get($table)->row_array();
        if (!$data) {
            $this->return_jsonp(dr_json(0, fc_lang('购物车记录不存在')));
        }

        if ($this->uid != $data['uid']) {
            $this->return_jsonp(dr_json(0, fc_lang('无权操作其他人的订单')));
        }
	...
	public function delete_all() {
        ...
        $ids = $this->input->post('ids');
        if (!$ids) {
            $this->return_jsonp(dr_json(0, fc_lang('没有选择商品')));
        }
        $_ids = array();
        foreach ($ids as $t) {
            if (intval($t)) {
	...
	public function update_num() {
		...
		if (!$data) {
		    ...
		    // 删除
            $this->db->where('uid', $this->uid)->where('id', $data['id'])->delete($this->order_model->prefix.'cart');
            $this->return_jsonp(dr_json(1, fc_lang('移除购物车')));
        }
		...
		// 更新入库
		$this->db->where('id', $id)->update($this->order_model->prefix.'cart', array(
			'quantity' => $num,
		));
    ...
    public function add() {
        ...
        if (!$this->uid) {
            echo dr_safe_replace($this->input->get('callback', TRUE)).'('.dr_json(0, fc_lang('请登录之后再操作')).')';exit;
        }
        ...
        if (!is_array($data)) {
            echo dr_safe_replace($this->input->get('callback', TRUE)).'('.dr_json(0, $data).')';exit;
        }

        if ($data['uid'] == $this->uid) {
            echo dr_safe_replace($this->input->get('callback', TRUE)).'('.dr_json(0, fc_lang('不允许购买自己的商品')).')';exit;
        }
		...
		echo dr_safe_replace($this->input->get('callback', TRUE)).'('.dr_json(1, $num).')';exit;
    }

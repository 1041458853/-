...
class Sell extends M_Controller {
    ...
    public function __construct() {
        ...
        if (!@in_array($this->groupid, $this->mconfig['permission']['sell']['use'])) {
            if ($this->groupid) {
                $this->member_msg(fc_lang('您无权限使用商家功能'));
            } else {
                $this->member_msg(fc_lang('您还没有登录，无权限使用'), dr_member_url('login/index'), 3, 2);
            }
        }
    }
    ...
    public function comment() {
        ...
        if ($data['order_status'] != 3) {
            $this->member_msg(fc_lang('订单状态不匹配'));
        }

...
class Mconfig extends M_Controller {
    ...
    public function permission() {
        ...
        $this->template->assign(array(
			...
			'menu' => $this->get_menu_v3(array(
                fc_lang('权限配置') => array('order/admin/mconfig/'.$this->router->method, 'user'),
                fc_lang('更新缓存') => array('admin/module/cache/dir/order', 'refresh'),
            )),
			'group' => $group,
        ));
    ...
    public function paytype() {
        ...
        $this->template->assign(array(
            ...
            'data' => $this->setting[SITE_ID][$this->router->method],
			'menu' => $this->get_menu_v3(array(
                fc_lang('付款方式') => array('order/admin/mconfig/'.$this->router->method, 'rmb'),
                fc_lang('更新缓存') => array('admin/module/cache/dir/order', 'refresh'),
            )),
        ));
        $this->template->display('mconfig_'.$this->router->method.'.html');
    ...
    public function notice() {
        ...
        $this->template->assign(array(
            ...
            'data' => $this->setting[SITE_ID][$this->router->method],
			'menu' => $this->get_menu_v3(array(
                fc_lang('订单通知') => array('order/admin/mconfig/'.$this->router->method, 'volume-off'),
                fc_lang('更新缓存') => array('admin/module/cache/dir/order', 'refresh'),
            )),
        ));
        $this->template->display('mconfig_'.$this->router->method.'.html');

...
class Buy extends M_Controller {
    ...
    public function index() {
        ...
        foreach ($ids as $id) {
            ...
            if ($row) {
                if ($row['buy_uid'] && $row['buy_uid'] != $this->uid) {
                    // 购买者判断
                    $this->msg(fc_lang('您无权限操作此订单'));
                } elseif ($row['pay_status'] != 0) {
                    // 状态判断
                    $this->msg(fc_lang('此订单已经付款'));
                }
                $sn[] = $row['sn'];
                $order[] = $id;
                $order_data[] = $row;
        ...
        // 付款方式不一致
        if (count($pay_type) > 1) {
            $this->msg(fc_lang('组合订单付款方式不一致，无法批量付款'), dr_member_url('order/home/index'), 2);
        } else {
        ...
        if ($price > 0 || $score > 0) {
            ...
            if ($pay_type == 1) {
                ...
                foreach ($order_data as $order) {
                    ...
                    if ($order['order_status'] != 2) {
                        ...
                        // 记录日志
                        $this->order_model->log($order, fc_lang('买家已下单'));
                    }
                }
                ...
                if (IS_POST) {
                    ...
                    if ($is_score || $type == 'yu') { // 当存在虚拟币时强制余额支付
                        if ($price > 0 && (!($this->member['money'] > 0) || $pay_price > 0)) {
                            // 判断rmb 余额不足
                            $this->msg(fc_lang('账户余额不足，当前余额￥%s', number_format($this->member['money'], 2)));
                        } elseif ($score > 0 && (!($this->member['score'] > 0) || $pay_score > 0)) {
                            // 判断虚拟币 余额不足
                            $this->msg(fc_lang('账户'.SITE_SCORE.'余额不足，当前'.SITE_SCORE.'%s', intval($this->member['score'])));
                        } else {
                            // 直接扣款
                            $this->order_model->pay($order_data, $price, 0);
                            $this->msg(fc_lang('付款成功'), count($order) > 1 ? dr_member_url('order/home/index') : dr_member_url('order/home/info', array('id'=>$order[0])), 2);
                        }
                    }
                    // 网银支付
                ...
                if (IS_POST) {
                    foreach ($order_data as $order) {
                        ...
                        // 记录日志
                        $this->order_model->log($order, fc_lang('买家已通过银行转账的方式付款'));
                    }
                    $this->msg(fc_lang('提交成功，我们会在1-3内处理转账信息'), count($order_data) > 1 ? dr_member_url('order/home/index') : dr_member_url('order/home/info', array('id'=>$order_data[0]['id'])), 2);
                }
            ...
            $this->msg(fc_lang('付款成功'), count($order_data) > 1 ? dr_member_url('order/home/index') : dr_member_url('order/home/info', array('id'=>$order_data[0]['id'])), 2);
        }
    }

...
class Order_model extends CI_Model {
    ...
    public function notice($id, $order, $buy = 0, $sell = 0) {
        ...
        if (isset($cfg['email']) && $cfg['email']) {
            ...
            if ($buy && $m = dr_member_info($buy)) {
                $this->ci->sendmail_queue($m['email'], fc_lang('订单%s提醒', $order['sn']), nl2br($content));
            }
            // 通知商家
    ...
    public function wc($order) {
        ...
        if ($order['order_price'] > 0 || $order['order_score'] >0) {
            if ($order['order_score']) {
                // 将虚拟币打给卖家
                $this->member_model->update_score(1, $order['sell_uid'], intval($order['order_score']), '', fc_lang('商品销售收入，订单号：%s', '<a href="'.SITE_URL.'index.php?s=member&mod=order&c=home&m=info&id='.$order['id'].'" target="_blank">'.$order['sn'].'</a>'));
            } else {
                // 将钱打给卖家
                ...
                $this->pay_model->add(
                    $order['sell_uid'],
                    $order['order_price'],
                    fc_lang('商品销售收入，订单号：%s', '<a href="'.SITE_URL.'index.php?s=member&mod=order&c=home&m=info&id='.$order['id'].'" target="_blank">'.$order['sn'].'</a>')
                );
            }
        }
        ...
        foreach ($orders as $order) {
            if ($order['order_price'] > 0 || $order['order_score'] > 0) {
                if ($order['order_score']) {
                    // 虚拟币支付时
                    $pay_id = 0;
                    $this->member_model->update_score(1, $order['buy_uid'], -intval($order['order_price']), '', fc_lang('购物消费，订单号：%s', '<a href="'.SITE_URL.'index.php?s=member&mod=order&c=home&m=info&id='.$order['id'].'" target="_blank">'.$order['sn'].'</a>'));
                } else {
                    // 扣钱
                    $this->load->model('pay_model');
    ...
    public function shopping($order, $data) {
        ...
        // 记录日志
        $this->log($order, fc_lang('卖家已发货，承运：%s，运货单号：%s', $data['name'], $data['sn']));
        // 发货之后提醒买家

...
class M_Controller extends D_Common {
    ...
    public function print_info() {
        ...
        foreach ($ids as $id) {
            ...
            if (!$order) {
                $this->member_msg(fc_lang('订单不存在'));
            }

            // 权限验证
            if (!IS_ADMIN && $order['sell_uid'] != $this->uid) {
                $this->member_msg(fc_lang('您无权限操作此订单'));
            }

            if (is_file($file)) {
                require $file;
            } else {
                $this->msg(fc_lang('订单打印模板不存在'));
            }
        }
    ...
    protected function _admin_list($name, $status = 0, $transfer = 0) {
        ...
        if (IS_POST) {
            ...
            if ($_POST['action'] == 'print') {
                ...
                $this->admin_msg(fc_lang('正在执行中...'), dr_url('order/home/print_info', array('id' => @implode(',', $ids))), 2, 2);
        ...
        list($list, $param) = $this->order_model->limit_page($param, $status, $transfer); // 分页查询

...
class M_Operate extends M_Controller {
    ...
    public function price() {
        ...
        if (!$data) {
            ...
            $this->template->assign('order_error', fc_lang('订单不存在'));
        ...
        if (!IS_ADMIN && $data['sell_uid'] != $this->uid) {
            if (IS_AJAX) {
                echo $this->callback_json(array(
                    'msg' => fc_lang('您无权限操作此订单'),
                    'code' => 0
                ));exit;
            }
            $this->template->assign('order_error', fc_lang('您无权限操作此订单'));
            $this->template->display('operate_price.html');exit;
        }
        ...
        if (in_array($data['pay_status'], array(1,2,3))) {
            ...
            if (IS_AJAX) {
                echo $this->callback_json(array(
                    'msg' => fc_lang('此订单正在付款中，无法修改价格'),
                    'code' => 0
                ));exit;
            }
            $this->template->assign('order_error', fc_lang('此订单正在付款中，无法修改价格'));
            $this->template->display('operate_price.html');exit;
        }
        ...
        if ($this->input->get('submit')) {
            ...
            $this->order_model->log($data, fc_lang('卖家把订单价格由 ￥%s 改为 ￥%s', number_format($old, 2), number_format($price, 2)));
            // 修改价格提醒买家
    ...
    public function save_note() {
        ...
        $this->db->where('id', $id)->update($this->order_model->prefix.'data_'.$data['tableid'], array(
            'sell_note' => $note,
        ));
        echo $this->callback_json(array(
            'msg' => $note,
    ...
    public function shipping() {
        ...
        if ($data['order_status'] != 2) {
            ...
            $this->template->assign('order_error', fc_lang('订单状态不匹配'));
            $this->template->display('operate_shipping.html');exit;
        }

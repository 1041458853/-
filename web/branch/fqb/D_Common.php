...
class D_Common extends CI_Controller {
	...
	public function  __construct (){
		...
		$this->_init_variable ();
		$this->lang ->load ('my');
		$this->hooks ->call_hook ('finecms-init');
		// poscms 初始化完毕的钩子
		$this->hooks ->call_hook ('poscms-init');
		// poscms 初始化完毕的钩子
		...
		$this->template ->assign (array ('get'=>$this->input ->get (NULL,TRUE),'member'=>$this->member ,'dirname'=>APP_DIR ,'my_city'=>$this->my_city ,'markrule'=>$this->markrule ,'site_info'=>$this->site_info ,'is_mobile'=>$this->template ->mobile ,'my_position'=>$this->my_position ,'member_rule'=>$this->member_rule ,'module_rule'=>$this->module_rule ,));
	}
	public function  baidu_fanyi ($string){
		...
		if (isset($this->_fanyi [$string])){
			return SITE_LANGUAGE =='en'?ucfirst($this->_fanyi [$string]):
			$this->_fanyi [$string];
		}
	...
	public function  clear_cache ($name){
		...
		// 模块缓存时，清除所有相关文件
		if ($name=='module' && dr_is_memcache () && $this->cache ->memcached ->is_supported ()){
	...
	public function  get_cache_data ($name){
		...
		return dr_is_memcache () && $this->cache ->memcached ->is_supported ()?$this->cache ->memcached ->get (SYS_KEY .$name): @$this->cache ->file ->get ($name);
	}
	/**
     * 临时数据缓存
     *
	...
	public function  set_cache_data ($name,$data,$ttl=10){
		...
		dr_is_memcache () && $this->cache ->memcached ->is_supported ()?$this->cache ->memcached ->save (SYS_KEY .$name,$data,$ttl):
	...
	private function  _init_variable (){
		...
		define('IS_POST',$_SERVER['REQUEST_METHOD']=='POST' && count($_POST)?TRUE:
		FALSE);
		define('SYS_TIME',$_SERVER['REQUEST_TIME']?$_SERVER['REQUEST_TIME']:
		time());
		define('SITE_PATH','/');
		...
		// 全局化站点变量
		$config1=require WEBPATH .'config/system.php';
		// 加载网站系统配置文件
		$config2=require WEBPATH .'config/version.php';
		// 加载系统版本更新文件
		$config1['SYS_CMS']=$config1['SYS_CMS']?$config1['SYS_CMS']:
		$config2['DR_NAME'];
		$config1['SYS_NAME']=$config1['SYS_NAME']?$config1['SYS_NAME']:
		'POSCMS';
		...
		// 加载站点域名配置文件
		$sitecfg=directory_map (WEBPATH .'config/site/');
		// 加载全部站点的配置文件
		...
		foreach ($sitecfg as $file){
			...
			if (is_file(WEBPATH .'config/site/'.$file) && $id>0){
				...
				$this->site_info [$id]['SITE_PC']=$this->site_info [$id]['SITE_URL']=dr_http_prefix (($this->site_info [$id]['SITE_DOMAIN']?$this->site_info [$id]['SITE_DOMAIN']:
				DOMAIN_NAME ).SITE_PATH );
				$this->site_info [$id]['SITE_MURL']=dr_http_prefix (($this->site_info [$id]['SITE_MOBILE']?$this->site_info [$id]['SITE_MOBILE']:
				DOMAIN_NAME ).SITE_PATH );
			}
		}
		unset($sitecfg);
		...
		if (isset($domain[DOMAIN_NAME ]) && isset($this->site_info [$domain[DOMAIN_NAME ]])){
			...
			$this->site_info [$siteid]['SITE_M_URL']=$this->site_info [$siteid]['SITE_PC']=$this->site_info [$siteid]['SITE_URL'];
			// PC端为主域名
			$uri=isset($_SERVER['HTTP_X_REWRITE_URL']) && trim($_SERVER['REQUEST_URI'],'/')==SELF ?trim($_SERVER['HTTP_X_REWRITE_URL'],'/'):
			...
			elseif (isset($this->site_info [$siteid]['SITE_MOBILE']) && $this->site_info [$siteid]['SITE_MOBILE']){
				...
				elseif ($this->_is_mobile () && $this->site_info [$siteid]['SITE_MOBILE_OPEN'] && $this->site_info [$siteid]['SITE_MOBILE'] && DOMAIN_NAME ==$this->site_info [$siteid]['SITE_DOMAIN'] && !IS_ADMIN ){
		...
		if (!$config3['SYS_UPLOAD_DIR']){
			...
			$config3['SYS_ATTACHMENT_URL']=$config3['SYS_ATTACHMENT_URL']?$config3['SYS_ATTACHMENT_URL']:
			$config3['SITE_URL'].$config3['SYS_UPLOAD_DIR'];
		}else {
		...
		$config3['UEDITOR_IMG_ID']=md5($config3['SYS_ATTACHMENT_URL']);
		// 编辑器附件ID
		$config3['SITE_FID']=isset($config3['SITE_FID'])?$config3['SITE_FID']:
		0;
		$config3['SITE_LID']=isset($config3['SITE_LID'])?$config3['SITE_LID']:
		0;
		// 将配置文件数组转换为常量
		foreach ($config3 as $var=>$value){
			...
			elseif (($var=='SITE_THEME' && $value=='SITE_THEME') || ($var=='SITE_TEMPLATE' && $value=='SITE_TEMPLATE')){
				$value='default';
			}
		...
		// 定义模板目录
		define('TPLPATH',strpos(SYS_TEMPLATE ,'/')===0?SYS_TEMPLATE .'/':
		(is_dir(WEBPATH .(SYS_TEMPLATE ?SYS_TEMPLATE :
		'templates').'/')?WEBPATH .(SYS_TEMPLATE ?SYS_TEMPLATE :
		'templates').'/':
		WEBPATH .'templates/'));
		// 定义pc端域名
		...
		// 网站语言
		date_default_timezone_set('Etc/GMT'.(SITE_TIMEZONE >0?'-':
		'+').abs(SITE_TIMEZONE ));
		// 设置时区
		...
		define('IS_API_AUTH',$json);
		// 接口授权成功
		IS_API_AUTH  && !IS_POST  && exit($this->callback_json (array ('msg'=>'请用POST方式提交','code'=>0,)));
		// 获取当前的登录记录
		...
		// 当前会员组的权限信息
		$this->markrule =$this->member ?$this->member ['mark']:
		0;
		$this->member_rule =isset($MEMBER['setting']['permission'][$this->markrule ])?$MEMBER['setting']['permission'][$this->markrule ]:
		NULL;
		// 当前会员权限规则
		// 当前会员的模块栏目权限规则
		$s=0;
		...
		// 空间黄页地址
		define('SPACE_URL',$MEMBER['setting']['space']['domain']?dr_http_prefix ($MEMBER['setting']['space']['domain'].'/'):
		($MEMBER['setting']['space']['rule']['space']?$MEMBER['setting']['space']['rule']['space']:
		SITE_URL .'index.php?s=space'));
		// 会员风格和模板目录
		if ($this->member ){
		...
		// 网站风格地址
		define('HOME_THEME_PATH',strpos(SITE_THEME ,'http')===0?trim(SITE_THEME ,'/').'/':
		THEME_PATH .SITE_THEME .'/');
		define('HOME_MOBILE_PATH',HOME_THEME_PATH );
		// 会员中心风格地址
		define('MEMBER_THEME',SITE_THEME );
		define('MEMBER_TEMPLATE',SITE_TEMPLATE );
		define('MEMBER_THEME_PATH',strpos(MEMBER_THEME ,'http')===0?trim(MEMBER_THEME ,'/').'/':
		THEME_PATH .MEMBER_THEME .'/');
		define('MEMBER_MOBILE_PATH',MEMBER_THEME_PATH );
		$this->load ->library ('template');
		$this->template ->mobile =$this->mobile ;
		// 当网站处于关闭状态时，非管理员无法访问
		defined('SITE_CLOSE') && SITE_CLOSE  && !IS_ADMIN  && !$this->member ['adminid'] && $this->router ->class !='api' && !($this->router ->class =='home' && $this->router ->method =='captcha') && !($this->router ->class =='home' && $this->router ->method =='shauth') && $this->msg (SITE_CLOSE_MSG );
		...
		if (IS_MEMBER ){
			...
			elseif (defined('DR_UEDITOR') && is_dir(WEBPATH .'api/'.DR_UEDITOR )){
				...
				$notice=array ();
				// 统计未读短消息
				$notice['app/pms/index']=dr_is_app ('pms')?$this->db ->where ('uid',$this->uid )->where ('isnew',1)->count_all_results ('pm_members'):
				0;
			...
			$this->pagesize =$MEMBER['setting']['pagesize']?(int )$MEMBER['setting']['pagesize']:
			10;
			if ($menu['data']){
				foreach ($menu['data'] as $i=>$t){
					// 不存在时删除
					if (!isset($menu['data'][$i]['left']) || !$menu['data'][$i]['left']){
						unset($menu['data'][$i]);
					}
					// 筛选模块权限
					if ($t['mark']=='m_mod'){
						...
						foreach ($menu['data'][$i]['left'] as $one){
							...
							if ($dir=='order'){
								...
								$is_buy=isset($perm['buy']['use'])?$perm['buy']['use']:
								array ();
								$is_sell=isset($perm['sell']['use'])?$perm['sell']['use']:
								array ();
								if ($one['link']){
									foreach ($one['link'] as $o){
										// 无权限使用商家中心的过滤
										if (($o['uri']==$dir.'/home/index' && !in_array($this->member ['groupid'],$is_buy)) || ($o['uri']==$dir.'/sell/index' && !in_array($this->member ['groupid'],$is_sell))){
											unset($menu['data'][$i]['left'][$pid]);
										}
									}
								}
							...
							elseif ($MOD[$dir]['is_system'] && $one['link']){
								...
								foreach ($one['link'] as $o){
									if (strpos($o['uri'],$dir.'/form_')!==false){
										...
										if ($name && $this->get_cache ('module-'.SITE_ID .'-'.$dir,'form',$name,'permission',$this->markrule ,'disabled')){
											unset($menu['data'][$i]['left'][$pid]['link'][$o['id']]);
										}
									}
								}
					...
					if ($t['mark']=='m_space'){
						...
						if ((!MEMBER_OPEN_SPACE  || !$this->member ['allowspace']) && isset($menu['data'][$i])){
							unset($menu['data'][$i]);
						}else {
							foreach ($menu['data'][$i]['left'] as $o=>$one){
								if (isset($one['link']) && $one['link']){
									foreach ($one['link'] as $k=>$c){
										...
										if (!$this->member ['spacedomain'] && isset($menu['data'][$i]['left']) && $menu['data'][$i]['left'] && $c['uri']=='space/space/domain'){
			...
			// 增加内容扩展菜单的右侧关联显示
			$cur=strpos($uri,APP_DIR .'/extend/')!==FALSE || preg_match('/^'.APP_DIR .'\/form_[0-9]+_[0-9]+\/listc/Ui',$uri)?$menu['uri'][APP_DIR .'/home/index']:
			(isset($menu['uri'][$uri])?$menu['uri'][$uri]:
			$menu['uri'][str_replace(strrchr($uri,'/'),'/index',$uri)]);
			// 当模块下不存在左侧栏目时默认选中列表页
			$cur=!$cur && APP_DIR  && (is_dir(FCPATH .'module/'.APP_DIR ) || is_dir(FCPATH .'app/'.APP_DIR )) && isset($menu['uri'][APP_DIR .'/home/index'])?$menu['uri'][APP_DIR .'/home/index']:
			$cur;
			$this->template ->assign (array ('uid'=>$this->uid ,'menu'=>$menu['data'],'menu_id'=>$cur['id'],// 当前URI对应的菜单id
			'menu_pid'=>$cur['pid'],// 当前URI对应的父级菜单id
			'menu_tid'=>$cur['tid'],// 当前URI对应的顶级菜单id
			'meta_name'=>$cur['name'],// 当前菜单名称作为标题名称
			'regverify'=>$MEMBER['setting']['regverify'],'member_rule'=>$this->member_rule ,));
			// 登录判断
			if ($this->_is_login_member ()){
				...
				// 游客发布权限不验证
				$verify=APP_DIR !='member' && in_array($uri,array ('home-add','home-field')) && !$this->member ?FALSE:
				TRUE;
				// 没有登录时
				if (!$this->member ){
					if (empty($_GET['login_fail'])){
				...
				// 已经登录时
				if ($this->uid ){
					$this->hooks ->call_hook ('init_member');
					// 会员中心初始化的钩子
					...
					if ((APP_DIR !='member' || ($uri!='home-index' && $uri!='pm-webchat'))){
						...
						elseif ($MEMBER['setting']['auth'] && !$this->member ['is_auth'] && $this->router ->class !='account' && APP_DIR !='auth'){
							// 是否强制实名认证
							if (!dr_is_app ('auth')){
		...
		elseif (IS_ADMIN ){
			...
			// 权限判断
			!$this->is_auth ($uri) && (IS_AJAX ?exit(fc_lang ('您无权限操作(%s)%s',$uri)):
	...
	private function  _is_login_member (){
		...
		if (!defined('DR_UEDITOR') && !defined('DR_PAY_ID') && !defined('DISCUZ_ROOT') && !in_array($this->router ->class ,array ('register','login','api','sns'))){
			return 1;
		}
		return 0;
	}
	...
	public function  is_auth ($uri){
		...
		$uri=trim(substr_count($uri,'/')==1?'admin/'.$uri:
		$uri,'/');
		// 后台首页不验证
		if (in_array($uri,array ('admin/home/main','admin/home/index','admin/home/clear','admin/home/cache','admin/root/my',))){
			return TRUE;
		}
		// 判断对模块的栏目是否可管理
		...
		// return FALSE;
		// }
		$role=is_array($this->admin ['color']) && $this->admin ['color']?$this->admin ['color']:
		array_merge($this->admin ['role']['system'],$this->admin ['role']['module'],$this->admin ['role']['application']);
		if (!$role){
			return FALSE;
		}
		// uri在当前角色的权限列表中就验证通过
		...
		$route=trim($route,'/');
		// 针对空间内容模型的
		if (in_array($route,array ('space/admin/content/edit','space/admin/content/del','space/admin/content/add',))){
		...
		if (in_array($route,array ('admin/field/add','admin/field/del','admin/field/edit',))){
			return TRUE;
		}
		$this->_link =$this->_link ?$this->_link :
		$this->dcache ->get ('link');
		$this->_auth =$this->_auth ?$this->_auth :
		$this->auth_model ->get_auth_all ();
	...
	protected function  get_menu_v3 ($menu){
		...
		foreach ($menu as $name=>$t){
			...
			$_first;
			// 快捷菜单
			$_quick[]=array ('url'=>$url,'name'=>$name,'icon'=>'<i class="fa fa-'.$t[1].'"></i>',);
			// 生成链接
			$_link.='<li> <a href="'.$url.'" '.$class.'>'.($t[1]?'<i class="fa fa-'.$t[1].'"></i> ':
			'').$name.'</a> <i class="fa fa-circle"></i> </li>';
			$_i++;
		}
		// 修改菜单时
		...
		return array ('name'=>$_name,'link'=>$_link,'quick'=>$_quick,);
	}
	/**
     * 后台操作界面中的顶部导航菜单
     *
	...
	protected function  get_menu ($menu){
		...
		$_str=$_mark?str_replace('{FIR}','on',$_str):
		$_str;
		return $_mark?str_replace('{MARK}',' class="on"',$_str):
		str_replace('{MARK}','',$_str);
	}
	...
	public function  get_member_pagination ($url,$total){
		$this->load ->library ('pagination');
		$config=array ();
		is_file(WEBPATH .'config/pagination_member.php')?include WEBPATH .'config/pagination_member.php':
	...
	protected function  is_admin_login (){
		...
		if (!is_array($data)){
			if (IS_ADMIN ){
				if ($data==0){
					IS_AJAX ?exit(dr_json (0,fc_lang ('会员不存在'))):
					$this->admin_msg (fc_lang ('会员不存在'));
				}
				elseif ($data==-3){
					IS_AJAX ?exit(dr_json (0,fc_lang ('您无权限登录管理平台'))):
					$this->admin_msg (fc_lang ('您无权限登录管理平台'));
				}
				elseif ($data==-4){
					IS_AJAX ?exit(dr_json (0,fc_lang ('您无权限登录该站点'))):
					$this->admin_msg (fc_lang ('您无权限登录该站点'));
				}
			}else {
				return $data;
			}
	...
	public function  admin_msg ($msg,$url='',$mark=0,$time=1){
		if (IS_API_AUTH ){
			echo $this->callback_json (array ('msg'=>$msg,'code'=>$mark?1:
			0));
		}else {
			IS_AJAX  && exit(dr_json (($mark?1:
			0),$msg,$url));
			$this->template ->assign (array ('msg'=>$msg,'url'=>$url,'time'=>$time,'mark'=>$mark,));
	...
	public function  member_msg ($msg,$url='',$mark=0,$time=1,$ajax=FALSE){
		if (IS_API_AUTH  || defined('SELECT_API_AUTH')){
			...
			if ($ajax || IS_AJAX ){
				...
				$this->template ->assign (array ('msg'=>$msg,'url'=>$url,'time'=>$time,'mark'=>$mark,'meta_name'=>fc_lang ('提示信息')));
				IS_MEMBER ?$this->template ->display ('msg.html','member'):
	...
	protected function  pay_msg ($msg,$url='',$mark=0){
		$this->template ->assign (array ('url'=>$url,'msg'=>$msg,'mark'=>$mark,));
	...
	public function  captcha (){
		...
		$this->captcha ->width =$this->input ->get ('width')?$this->input ->get ('width'):
		80;
		$this->captcha ->height =$this->input ->get ('height')?$this->input ->get ('height'):
		30;
		$this->session ->unset_userdata ('captcha');
	...
	public function  get_system_run_info (){
		return dr_lang ('页面执行时间%s秒，共%s个查询语句，消耗内存%s',$this->benchmark ->elapsed_time ('total_execution_time_start','total_execution_time_end'),count($this->db ->queries ),str_replace(' ','',dr_format_file_size (memory_get_usage()/4)));
	}
	/**
     * 表单提交数据验证和过滤
     *
	...
	protected function  validate_filter ($_field,$_data=array ()){
		...
		foreach ($_field as $field){
			...
			// 编辑器默认关闭xss
			$validate['xss']=!isset($validate['xss']) && $field['fieldtype']=='Ueditor'?1:
			$validate['xss'];
			// 从表单获取值
			$this->post [$name]=$value=$this->input ->post ("data[$name]",$validate['xss']?FALSE:
			TRUE);
			// 验证必填字段
			...
			// 正则验证
			if (!is_array($value) && strlen($value) && isset($validate['pattern']) && $validate['pattern'] && !preg_match($validate['pattern'],$value)){
				return array ('error'=>$name,'msg'=>$field['name'].'：'.($validate['errortips']?$validate['errortips']:
				fc_lang ('格式不正确')));
			}
			// 函数/方法校验
					...
					if (method_exists($this->dvalidate ,$method)){
						...
						if ('check_member'==$method && $value=='guest'){
							...
							if ($_param && $_value=explode(',',$_param)){
								foreach ($_value as $t){
									$param[$t]=isset($_POST['data'][$t])?$this->input ->post ("data[$t]"):
									$t;
								}
							}
	...
	public function  attachment_handle ($uid,$related,$field,$_data=NULL,$update=TRUE){
		...
		foreach ($field as $f){
			...
			$files && $attach_id=$attach_id?array_merge($attach_id,$files):
			$files;
		}
		// 筛选删除的附件
		foreach ($field as $f){
			...
			$attach=$attach?array_merge($attach,$add_id):
			$add_id;
		}
		$attach=$attach?array_merge($attach,$attach_id):
		$attach_id;
		if (count($attach)==0){
			return NULL;
		}
	...
	public function  new_field_input ($field,$data=NULL,$cat=FALSE,$id='id',$html='',$is_diy=0){
		...
		// 字段合并分组筛选
		foreach ($field as $t){
			if ($t['fieldtype']=='Merge' && preg_match_all('/\{(.+)\}/U',$t['setting']['option']['value'],$value)){
				foreach ($value[1] as $v){
					$merge[$v]=$t['fieldname'];
				}
			}
		...
		foreach ($field as $t){
			...
			elseif (IS_ADMIN  && $this->member ['adminid']>1 && @in_array($this->member ['adminid'],$t['setting']['show_admin'])){
				continue ;
			}
			...
			if (is_object($obj)){
				// 百度地图特殊字段
				$obj->remove_div =0;
				$value=$t['fieldtype']=='Baidumap'?($data[$t['fieldname'].'_lng'] && $data[$t['fieldname'].'_lat']?$data[$t['fieldname'].'_lng'].','.$data[$t['fieldname'].'_lat']:
				$data[$t['fieldname']]):
				$data[$t['fieldname']];
				if (isset($group[$t['fieldname']])){
					// 属于分组字段,重新获取字段表单
					$obj->remove_div =1;
					$mygroup[$t['fieldname']]=$obj->input ($t['name'],$t['fieldname'],$t['setting'],$value,isset($data[$id])?$data[$id]:
					0);
				}
				elseif (isset($merge[$t['fieldname']])){
					// 属于合并字段
					$input=$obj->input ($t['name'],$t['fieldname'],$t['setting'],$value,isset($data[$id])?$data[$id]:
					0);
		...
		if ($merge){
			$html='					    </div>					</div>				</div>                <div class="portlet light bordered" id="dr_{name}">                    <div class="portlet-title mytitle">                        <div class="caption"><span class="caption-subject font-green">{text}</span></div>                    </div>                    <div class="portlet-body">                        <div class="form-body">{value}';
			if (!IS_ADMIN ){
			...
			foreach ($data as $mname=>$value){
				...
				if ($value){
					...
					$myfield=$code?str_replace('{merge_'.$mname.'}',str_replace(array ('{text}','{name}','{value}'),array (fc_lang ($field[$mname]['name']),$mname,$code),$html),$myfield):
					str_replace('{merge_'.$mname.'}','',$myfield);
				}
			}
		}
		if ($mygroup){
			...
			if (isset($fields[$n]) && $fields[$n]){
				...
				elseif (SITE_MOBILE !==TRUE && $n=='content' && $fields[$n]['fieldtype']=='Ueditor' && strpos($value,'<div name="dr_page_break" class="pagebreak">')!==FALSE && preg_match_all('/<div name="dr_page_break" class="pagebreak">(.*)<\/div>/Us',$value,$match) && preg_match('/(.*)<div name="dr_page_break"/Us',$value,$frist)){
					...
					foreach ($match[0] as $i=>$t){
						...
						$body=preg_match('/(.*)<div name="dr_page_break"/Us',$value,$match_body)?$match_body[1]:
						$value;
						$title[$page]=trim($match[1][$i]);
						$content[$page]['title']=trim($match[1][$i])?trim($match[1][$i]):
						fc_lang ('第%s页',$page);
				...
				elseif (SITE_MOBILE !==TRUE && $fields[$n]['fieldtype']=='Ueditor' && strpos($value,'<p class="pagebreak">')!==FALSE && preg_match_all('/<p class="pagebreak">(.*)<\/p>/Us',$value,$match) && preg_match('/(.*)<p class="pagebreak">/Us',$value,$frist)){
					// 编辑器分页 新版
					$page=1;
					$content=$title=array ();
					$data['_'.$n]=$value;
					...
					foreach ($match[0] as $i=>$t){
						$page++;
						$value=str_replace($content[$page-1]['body'].$t,'',$value);
						$body=preg_match('/(.*)<p class="pagebreak"/Us',$value,$match_body)?$match_body[1]:
						$value;
						$title[$page]=trim($match[1][$i]);
						$content[$page]['title']=trim($match[1][$i])?trim($match[1][$i]):
						fc_lang ('第%s页',$page);
	...
	protected function  _check_admin_verify ($data,$category){
		...
		if ($this->admin ['adminid']>1){
			...
			$mark=$user['groupid']<3?$user['groupid']:
			($user['groupid'].'_'.$user['levelid']);
	...
	protected function  _module_post_catid ($module,$markrule=NULL){
		...
		$catid=array ();
		$markrule=$markrule?$markrule:
		$this->markrule ;
		foreach ($module['category'] as $cat){
			// 跳过有下级栏目的判断
			if ($module['setting']['pcatpost']?0:
			$cat['child']){
				continue ;
			}
	...
	public function  _is_module_admin ($uri){
		...
		foreach ($MOD[$dir]['category'] as $cat){
			// 跳过有下级栏目的判断
			if ($MOD[$dir]['setting']['pcatpost']?0:
			$cat['child']){
				continue ;
			}
			// 当栏目中存在一项是管理就标识为管理状态
			if (isset($cat['setting']['admin'][$this->admin ['adminid']]['show']) && $cat['setting']['admin'][$this->admin ['adminid']]['show']==1){
				return TRUE;
			}
		}
		return FALSE;
	...
	public function  sendmail_queue ($tomail,$subject,$message){
		...
		$this->cron_model ->add (1,array ('tomail'=>$tomail,'subject'=>$subject,'message'=>$message,));
		return TRUE;
	}
	...
	protected function  _check_write_able ($pathfile){
		...
		$isDir=in_array(substr($pathfile,-1),array ('/','\\'))?TRUE:
		FALSE;
		if ($isDir){
			if (is_dir($pathfile)){
	...
	protected function  api_template (){
		...
		$this->template ->assign (array ('get'=>$get,'params'=>$params,'dirname'=>$this->input ->get ('dirname')));
		$this->template ->assign ($get);
		$this->template ->assign ($params);
		$name=str_replace(array ('\\','/','..','<','>'),'',dr_safe_replace ($this->input ->get ('name',TRUE)));
		$this->template ->display (strpos($name,'.html')?$name:
		$name.'.html');
	...
	protected function  api_favorite (){
		if (!$this->uid ){
			...
			if (isset($_GET['jsonp']) && $_GET['jsonp']){
				echo dr_safe_replace ($this->input ->get ('callback',TRUE)).'('.json_encode(array ('code'=>'1')).')';
				exit ;
			}else {
				exit('1');
			}
		...
		if (!$data){
			// 文档不存在
			if (isset($_GET['jsonp']) && $_GET['jsonp']){
				echo dr_safe_replace ($this->input ->get ('callback',TRUE)).'('.json_encode(array ('code'=>'2')).')';
				exit ;
			}else {
				exit('2');
			}
		...
		if ($eid){
			...
			if ($favorite){
				if ($is_delete){
					...
					$this->db ->where ('id',$favorite['id'])->update ($table,array ('url'=>$data2['url'],'title'=>$data['title'].' - '.$data2['name']));
				}
				// 更新成功
				if (isset($_GET['jsonp']) && $_GET['jsonp']){
					echo dr_safe_replace ($this->input ->get ('callback',TRUE)).'('.json_encode(array ('code'=>'3')).')';
					exit ;
				}else {
					exit('3');
				}
				...
				// 添加成功
				$this->db ->insert ($table,array ('eid'=>$eid,'cid'=>$mid,'uid'=>$this->uid ,'url'=>$data2['url'],'title'=>$data['title'].' - '.$data2['name'],'inputtime'=>SYS_TIME ,));
				// 更新数量
				$c=$this->db ->where ('eid',$eid)->count_all_results ($table);
				$this->db ->where ('id',$eid)->set ('favorites',$c)->update (SITE_ID .'_'.APP_DIR .'_extend');
				if (isset($_GET['jsonp']) && $_GET['jsonp']){
					echo dr_safe_replace ($this->input ->get ('callback',TRUE)).'('.json_encode(array ('code'=>'4')).')';
					exit ;
				}else {
					exit('4');
				}
			...
			if ($favorite){
				if ($is_delete){
					...
					$this->db ->where ('id',$favorite['id'])->update ($table,array ('url'=>$data['url'],'title'=>$data['title']));
				}
				// 更新成功
				if (isset($_GET['jsonp']) && $_GET['jsonp']){
					echo dr_safe_replace ($this->input ->get ('callback',TRUE)).'('.json_encode(array ('code'=>'3')).')';
				...
				// 添加成功
				$this->db ->insert ($table,array ('eid'=>0,'cid'=>$mid,'uid'=>$this->uid ,'url'=>$data['url'],'title'=>$data['title']?$data['title']:
				'','inputtime'=>SYS_TIME ,));
				// 更新数量
	...
	public function  sitemap (){
		...
		$cache_name=SITE_ID .'-'.(APP_DIR  && in_array(APP_DIR ,$module)?APP_DIR .'-sitemap':
		'sitemap');
		...
		if (!$cache_data){
			...
			if (APP_DIR ){
				...
				if (in_array(APP_DIR ,$module) && $mod['sitemap'] && $mod['is_system']){
					...
					$data=$db->where ('status',9)->select ('url,updatetime,inputtime')->order_by ('updatetime DESC')->limit (5000)->get (SITE_ID .'_'.APP_DIR )->result_array ();
					if ($data){
						foreach ($data as $t){
							$xml.=$this->_url_format ($t['url'],date('Y-m-d',$t['updatetime']?$t['updatetime']:
							$t['inputtime']));
						}
					}
				}
				...
				if ($module){
					foreach ($module as $dir){
						...
						if ($mod['sitemap'] && $mod['is_system']){
							...
							$data=$db->where ('status',9)->select ('url,updatetime,inputtime')->order_by ('updatetime DESC')->limit (5000)->get (SITE_ID .'_'.$dir)->result_array ();
							if ($data){
								foreach ($data as $t){
									$xml.=$this->_url_format ($t['url'],date('Y-m-d',$t['updatetime']?$t['updatetime']:
									$t['inputtime']));
			...
			$xml.='</urlset>'.PHP_EOL;
			$cache_data=$this->set_cache_data ($cache_name,$xml,SYS_CACHE_SITEMAP );
			// 网站地图缓存24小时
		}
		echo $cache_data;
		exit ;
	...
	public function  goto_404_page ($msg){
		...
		$this->template ->assign (array ('msg'=>$msg,'meta_title'=>$msg));
	...
	protected function  system_log ($action,$insert=0){
		...
		$data=array ('ip'=>$this->input ->ip_address (),'uid'=>$this->member ['uid'],'time'=>SYS_TIME ,'action'=>addslashes($action),'username'=>$this->member ['username'],);
	...
	public function  field_table ($table,$sid){
		...
		if (!$data){
			$field=$this->db ->where ('disabled',0)->where ('relatedname','table-'.$table)->where ('relatedid',$sid)->order_by ('displayorder ASC, id ASC')->get ('field')->result_array ();
			if ($field){
				foreach ($field as $f){
					$data[$f['fieldname']]=$f;
				}
	...
	public function  get_qrcode ($value,$uid=0,$errorCorrectionLevel='L',$matrixPointSize=5){
		...
		if ($uid){
			$logo=is_numeric($uid)?dr_avatar ($uid):
			$uid;
	...
	public function  get_mytable_field ($table,$siteid=0){
		...
		if (!$value){
			$field=$this->db ->where ('disabled',0)->where ('relatedid',$siteid)->where ('relatedname','table-'.$table)->order_by ('displayorder ASC,id ASC')->get ('field')->result_array ();
			if ($field){
				foreach ($field as $t){
	...
	protected function  _indexc (){
		$file=WEBPATH .'cache/index/'.(IS_MOBILE ?'mobile-':
		'').DOMAIN_NAME .'-home-'.max(intval($_GET['page']),1).'.html';
		// 系统开启静态首页、静态文件不存在时，才生成文件
		if (defined('SYS_AUTO_CACHE') && SYS_AUTO_CACHE  && SYS_CACHE_INDEX  && !is_file($file) && !SITE_CLOSE ){
			ob_start();
			$this->template ->assign (array ('indexc'=>1,'meta_title'=>SITE_TITLE ,'meta_keywords'=>SITE_KEYWORDS ,'meta_description'=>SITE_DESCRIPTION ,));
	...
	protected function  _module_init (){
		...
		define('MODULE_THEME_PATH',IS_SHARE ?HOME_THEME_PATH :
		(strpos($this->module ['theme'],'http://')===0?trim($this->module ['theme'],'/').'/':
		THEME_PATH .($this->module ['theme']?$this->module ['theme']:
		'default').'/'));
		// 设置模块模板
		...
		if (IS_MEMBER ){
			...
			if ($data){
				foreach ($data as $i=>$t){
					isset($t[$this->member ['mark']]) && $t[$this->member ['mark']] && $t['name'] && $this->flag [$i]=$t;
				}
			}
		}
		// 模块自身初始化
	...
	protected function  _set_back_url ($uri,$param=array (),$nuri=''){
		...
		$this->cache ->file ->save ($name,dr_url ($nuri?$nuri:
		$uri,$param),3600);
	}
	// 替换语言
	public function  replace_lang ($string){
		...
		return $this->replace_lang ?str_replace($this->replace_lang [1],$this->replace_lang [2],$string):
		$string;
	}
	public function  html_thumb ($p){
		...
		if ($info && in_array($info['fileext'],array ('jpg','gif','png','jpeg'))){
			...
			if (isset($info['remote']) && $info['remote']){
				...
				if ($size){
					$file_size=str_replace(basename($info['attachment']),basename($info['attachment'],'.'.$info['fileext']).'_'.$size.'.'.$info['fileext'],$info['attachment']);
					$file=is_file(SYS_UPLOAD_PATH .'/'.$file_size)?SYS_UPLOAD_PATH .'/'.$file_size:
					SYS_UPLOAD_PATH .'/'.$info['attachment'];
				}else {
		...
		// 覆盖图片
		$config['maintain_ratio']=(bool )$site['SITE_IMAGE_RATIO'];
		// 使图像保持原始的纵横比例
		// 水印判断
		if (isset($info['remote']) && $info['remote'] && !$site['SITE_IMAGE_REMOTE']?FALSE:
		((bool )$site['SITE_IMAGE_WATERMARK'] && $water?TRUE:
		FALSE)){
			// 水印参数
			$config['wm_type']=$site['SITE_IMAGE_TYPE']?'overlay':
			'text';
			...
			$config['wm_font_path']=WEBPATH .'statics/watermark/'.($site['SITE_IMAGE_FONT']?$site['SITE_IMAGE_FONT']:
			'default.ttf');
			$config['wm_font_color']=$site['SITE_IMAGE_COLOR']?str_replace('#','',$site['SITE_IMAGE_COLOR']):
			'#000000';
			// 图片模式
			$config['wm_opacity']=$site['SITE_IMAGE_OPACITY']?$site['SITE_IMAGE_OPACITY']:
			80;
			$config['wm_overlay_path']=WEBPATH .'statics/watermark/'.($site['SITE_IMAGE_OVERLAY']?$site['SITE_IMAGE_OVERLAY']:
			'default.png');
			// 生成图片的临时文件
	...
	protected function  _cache_name (){
		$data=array (0=>array ('site::cache','application::cache','auth::role_cache','system::email','system::verify','system::sysvar','system::urlrule','system::attachment','system::downservers','module::cache','member/member::cache',),);
		...
		foreach ($this->site_info  as $sid=>$t){
			$data[$sid]=array ('page::cache','form::cache','system::block','navigator::cache','tag::cache','weixin::cache',);
		}
		$data[0][]='menu::cache';
